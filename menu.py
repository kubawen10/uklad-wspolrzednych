...
def get_var(button):
    if button == 0:
        ...
        return {"x = ": None, "y = ": None, "name = ": None}
    elif button == "Circle":
        return {"x = ": None, "y = ": None, "r = ": None, "name = ": None}
    elif button == "Line":
...
class Panel:
    def __init__(self, x, width, win_height, surface):
        ...
        self.cur_input = ""
        self.msg = get_msg(self.mode)
        self.var = get_var(self.mode)
...
class Menu:
    def __init__(self, x, width, height, surface):
        ...
        buttony = btty(len(self.buttons), BUTTON_HEIGHT)
        linebutton = Button((buttonx, buttony, BUTTON_WIDTH, BUTTON_HEIGHT), "Line", COLOR_ACTIVE, COLOR_INACTIVE,
                             self.surface)
